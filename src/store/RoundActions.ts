import type { TournamentType } from '@/globals/types'
...
export function finishRound(roundIndex: number) {
  ...
  if (!newTournament.rounds[roundIndex].finishable) throw Error(`Round roundIndex: ${roundIndex} is not finishable`)
  ...
  if (state.gameElimination === 'KO') {
    ...
    if (roundIndex === 0) {
      ...
      finishFinalRound(newTournament)
    } else {
      if (roundIndex % 2 === 0) {
...
function finishOddRound(newTournament: TournamentType, roundIndex: number) {
  // In an odd round we have to put the loser of the winner round and pair them with the winner of the loser round
...
function finishFinalRound(newTournament: TournamentType) {
  const finalRound = newTournament.rounds[newTournament.rounds.length - 1]

  if (!finalRound.finishable) return

  finalRound.finished = true
}
